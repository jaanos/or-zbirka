...
\begin{small}
\begin{algorithmic}
\Function{RekonstruirajPot}{$d_G, \pred, t$}
    \State $u \gets t$
	\State {\sl pot} $\gets [t]$
	\While{$\pred[u] \ne \Null$}
        \State $u \gets \pred[u]$
...
Klic {\sc RekonstruirajPot}$(d_G, \pred, e)$
nato vrne seznam vozlišč, skozi katera je algoritem potoval,
da je opravil pot od $g$ do $e$ -- to so $[g, a, b, e]$.
...
vrne pot $[g, h, b, c, d, e]$ dolžine $24$.
...
$u$ & $v$ & $d_G(v)$ &$>$& $d_G(u)$ &$+$& $L_{uv}$ & $a$ & $b$ & $c$ & $d$ & $e$ & $f$ & $g$ & $h$ & $\pred[v]$ \\ \hline

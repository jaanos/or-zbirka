...
\primer{razdelilnik, ki služi delavnicam od $i$ do $j$
(če $i > 1$ ali $j < n$,
je voda do njega pritekla preko predhodnih razdelilnikov),
lahko vodo razdeli na cevi,
ki služita delavnicam od $i$ do $k$ in od $k+1$ do $j$ za nek $k$
($i \le k < j$).
Cena postavitve takega razdelilnika je $\sum_{h=i}^j p_h$.
Če je $i < k$ oziroma $k+1 < j$, so potrebni še nadaljnji razdelilniki.}
...
\begin{slika}
\pgfslika
\podnaslov{Postavitev razdelilnikov}
\end{slika}
\end{enumerate}
\end{odgovor}
\end{naloga}
